...
public class playerController : MonoBehaviour {
	...
	void Start () {
		...
		WalkHerePromptText = "walk here";
		GUItext = GameObject.FindWithTag ("CursorGUI").GetComponentsInChildren <Text>();
		GUIimages = GameObject.FindWithTag ("CursorGUI").GetComponentsInChildren <RawImage>();
	}
	...
	void updateCursors()
	{
		...
		//update cursor text if necessary
		if(hitWalkable){
		//	GUItext[0].text = WalkHerePromptText;
			toggleGUIVisibility(255f);
		}
		if(hitWalkable && (!rayHit || hitInteractive)){
			hitWalkable = false;
			toggleGUIVisibility(0.0f);
		}
		
		//set the walkHere cursor
		walkHerePrompt.enabled = (isWalking) ? false : true;
		walkHereCursor.transform.position = (hitWalkable && isWalking) ? walkHereCursorPosition : new Vector3(0.0f, -999.0f, 0.0f) ;
	}
	...
	void toggleGUIVisibility(float alpha) 
	{
		GUItext = GameObject.FindWithTag ("CursorGUI").GetComponentsInChildren <Text>();
		GUIimages = GameObject.FindWithTag ("CursorGUI").GetComponentsInChildren <RawImage>();

		foreach (Text text in GUItext) {
